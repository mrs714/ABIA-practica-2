...
class BookSelectorApp:
    def __init__(self, root):
        ...
        self.books = self.read_books_from_dataset("./Media/books.csv")
    ...
    def read_books_from_dataset(self, file_path):
        books = []
        with open(file_path, "r", newline="", encoding="utf-8") as file:
            ...
            for row in reader:
                ...
                # Replace empty image URLs with the placeholder URL
                books.append({
                    "title": title,
                    "author": author,
                    "series": series,
    ...
    def populate_books_frame(self, frame):
        for i, book in enumerate(self.books):
            ...
            label.bind("<Button-1>", lambda event, title=title: self.toggle_select_book(title))
    ...
    def toggle_select_book(self, title):
        for book in self.books:
            if book['title'] == title:
                book['selected'] = not book['selected']
                self.update_book_label(title)
                self.update_selected_books_listbox()
                break

    def update_book_label(self, title):
        for widget in self.root.winfo_children():
        ...
        if selected_index:
            ...
            for book in self.books:
                if book['title'] == selected_title:
                    book['selected'] = False
                    self.update_book_label(selected_title)
                    break
            self.update_selected_books_listbox()
