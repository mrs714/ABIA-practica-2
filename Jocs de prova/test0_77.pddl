(define (problem test0_77)
    (:domain books)
    (:objects
        book0 book1 book2 book3 book4 book5 book6 book7 book8 book9 book10 book11 book12 book13 book14 book15 book16 book17 book18 book19 book20 book21 book22 book23 book24 book25 book26 book27 book28 book29 book30 book31 book32 book33 book34 book35 book36 book37 book38 book39 book40 book41 book42 book43 book44 book45 book46 book47 book48 book49 book50 book51 book52 book53 book54 book55 book56 book57 book58 book59 book60 book61 book62 book63 book64 book65 book66 book67 book68 book69 book70 book71 book72 book73 book74 book75 book76 book77 book78 book79 book80 book81 book82 book83 book84 book85 book86 - book
        January February March April May June July August September October November December - month
    )
    (:init
        (= (number January) 0)
        (= (number February) 1)
        (= (number March) 2)
        (= (number April) 3)
        (= (number May) 4)
        (= (number June) 5)
        (= (number July) 6)
        (= (number August) 7)
        (= (number September) 8)
        (= (number October) 9)
        (= (number November) 10)
        (= (number December) 11)
        (predecessor book36 book1)
        (predecessor book18 book2)
        (predecessor book3 book79)
        (predecessor book4 book60)
        (predecessor book57 book6)
        (predecessor book6 book85)
        (predecessor book12 book7)
        (predecessor book69 book8)
        (predecessor book9 book63)
        (predecessor book10 book68)
        (predecessor book62 book14)
        (predecessor book15 book81)
        (predecessor book23 book17)
        (predecessor book19 book75)
        (predecessor book22 book20)
        (predecessor book49 book21)
        (predecessor book55 book24)
        (predecessor book51 book25)
        (predecessor book25 book47)
        (predecessor book82 book31)
        (predecessor book54 book32)
        (predecessor book33 book35)
        (predecessor book34 book52)
        (predecessor book40 book44)
        (predecessor book72 book41)
        (predecessor book42 book65)
        (predecessor book59 book43)
        (predecessor book45 book78)
        (predecessor book48 book70)
        (predecessor book77 book50)
        (predecessor book53 book64)
        (predecessor book56 book74)
        (predecessor book83 book66)
        (predecessor book66 book86)
        (read book0)
        (read book1)
        (read book13)
        (read book14)
        (read book19)
        (read book20)
        (read book22)
        (read book23)
        (read book25)
        (read book32)
        (read book33)
        (read book34)
        (read book37)
        (read book38)
        (read book43)
        (read book48)
        (read book55)
        (read book58)
        (read book59)
        (read book64)
        (read book67)
        (read book68)
        (read book70)
        (read book80)
        (read book81)
        (read book83)
        (to-read book2)
        (to-read book3)
        (to-read book5)
        (to-read book6)
        (to-read book7)
        (to-read book9)
        (to-read book10)
        (to-read book12)
        (to-read book21)
        (to-read book28)
        (to-read book29)
        (to-read book35)
        (to-read book45)
        (to-read book46)
        (to-read book49)
        (to-read book50)
        (to-read book51)
        (to-read book52)
        (to-read book54)
        (to-read book56)
        (to-read book63)
        (to-read book71)
        (to-read book72)
        (to-read book76)
        (to-read book85)
        (to-read book86)
    )
    (:goal (forall (?book - book) (imply (to-read ?book) (read ?book))))
)
